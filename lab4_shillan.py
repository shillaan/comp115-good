...
def draw_circles(t, size, amount):
    for _ in range(4):
        t.circle(size)
        size = size - amount
...
def draw_special(turtle, start_size, times, shrink):
    for i in range(times):
        draw_circles(turtle, start_size, shrink)
...
def draw_picture_nice():
    ...
    colors = ['crimson', 'orange', 'chartreuse', 'dodger blue', 'violet']
    decrease_amounts = [4, 5, 10, 19, 20]

    for i in range(len(colors)):
        t.color(colors[i])
        draw_special(t, 100, 6, decrease_amounts[i])
