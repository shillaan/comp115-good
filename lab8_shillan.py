...
"""
    ...
    4
    """
    return len(set(ids))  # set automatically keeps only unique IDs
...
assert students_id(['100', '100', '100']) == 1
assert students_id(['001', '002', '003', '004', '005', '006']) == 6
    """
...
assert students_id_occurrences(['100', '100', '100']) == {'100': 3}
assert students_id_occurrences(['001', '002', '003', '001', '002']) == {'001': 2, '002': 2, '003': 1}
"""
...
def word_frequency(paragraph):
    ...
    words = re.findall(r'\b\w+\b', paragraph)  # extract words
    freq = {}
    for word in words:
        if word in freq:
            freq[word] += 1
...
assert word_frequency("Hello hello") == {'Hello': 1, 'hello': 1}  # case-sensitive
assert word_frequency("Python is fun. Python is easy.") == {'Python': 2, 'is': 2, 'fun': 1, 'easy': 1}
