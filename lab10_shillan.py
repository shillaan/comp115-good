...
def create_eight_pixels_image():
    ...
    for row in range(2):
        for col in range(4):
            ...
            a = palette_pixels[i][3]
            eight_pixels_image.putpixel((col, row), (r, g, b, a))
            i += 1
...
def dim_eight_pixels_image():
    ...
    im_dim = Image.new("RGBA", (im.width, im.height))
    ...
    for row in range(im.height):
        for col in range(im.width):
            ...
            b = b // 2
            a = a // 2
            im_dim.putpixel((col, row), (r, g, b, a))
...
def blue_washed_image():
    ...
    im_blue = Image.new("RGBA", (im.width, im.height))
...
# Upload it to your github account, and copy the link to e-learn. That's all.import random
from PIL import Image


def create_red_one_pixel():
    one_pixel_image = Image.new('RGB', (1, 1))
    one_pixel_image.putpixel((0, 0), (255, 0, 0))
    one_pixel_image.save("red_one_pixel.png")
...
def create_eight_pixels_image():
    eight_pixels_image = Image.new('RGB', (4, 2))
    i = 0
    for row in range(2):
        for col in range(4):
            r = palette_pixels[i][0]
...
def blue_washed_image():
    im = Image.open("rick_and_morty.png").convert("RGB")
...
def decode_image():
    im = Image.open("rick_encoded.png").convert("RGB")
    im_secret = Image.new("RGB", (im.width, im.height))
